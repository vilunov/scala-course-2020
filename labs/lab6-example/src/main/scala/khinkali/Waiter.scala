...
object Waiter {
  ...
  case class RevealToChef(chefs: Vector[ActorRef[Chef.Command]],
    ...
    Behaviors.receive { (ctx, msg) =>
      msg match {
        ...
        case ProcessOrder(order, customer) =>
          ctx.self ! RevealToChef(chefs, order, customer)
          Behaviors.same

        case RevealToChef(notAsked, order, customer) =>
          notAsked.headOption match {
            case Some(chef) =>
              ctx.ask(chef, Chef.AcceptOrder(order, customer, _: ActorRef[Result])) {
                case Success(Result.Ok) => WaitForOrder
                case _ => RevealToChef(notAsked.tail, order, customer)
              }
              Behaviors.same
            case None =>
