...
package object ord {
  ...
  implicit class SyntaxPartialEquality[T](val left: T) extends AnyVal {
    def is(right: T)(implicit instance: PartialEquality[T]): Boolean = instance.equal(left, right)
  }

  implicit class SyntaxPartialOrder[T](val left: T) extends AnyVal {
    def <(right: T)(implicit instance: PartialOrd[T]): Boolean =
      instance.partialCompare(left, right).fold(false)(_ == OrdResult.Less)
